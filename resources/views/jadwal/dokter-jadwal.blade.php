    @extends('layouts.main-master')
    @section('content')
        <div class="content">
            <a href="{{ route('doctors.id', ['id' => $dokter['id']]) }}"><i class="fa fa-angle-left"></i> Back</a>
            <hr>
            @include('layouts.flash-alert')
            @isset($_GET['add'])
                <div class="row">
                    <div class="col-md-6">
                        <div class="card-box">
                            <div class="row">
                                <div class="col-sm-7 col-6">
                                    <h4 class="card-title">Tambah Jadwal</h4>
                                </div>
                                <div class="col-sm-5 col-6 text-right m-b-30">
                                    <a href="{{ route('jadwal', ['id' => $dokter['id']]) }}" class="">
                                        Cencel
                                    </a>
                                </div>
                            </div>
                            <form action="{{ route('jadwal.create') }}" method="POST">
                                @csrf
                                <input type="hidden" name="id_dokter" value="{{ $dokter['id'] }}">
                                <div class="form-group">
                                    <label>Hari <span class="text-danger">*</span></label>
                                    <select class="form-control {{ $errors->has('hari') ? 'is-invalid' : '' }}" name="hari">
                                        <option value="">-- Select --</option>
                                        @foreach ($days as $day)
                                            <option value="{{ $day }}">{{ $day }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('hari'))
                                        <span class="text-danger">{{ $errors->first('hari') }}</span>
                                    @endif
                                </div>
                                <div class="form-group">
                                    <label>Jam <span class="text-danger">*</span></label>
                                    <div class="d-flex flex-md-row flex-sm-column flex-column">
                                        <div class="w-100">
                                            <div class="time-icon">
                                                <input
                                                    class="form-control timepicker {{ $errors->has('jam_buka') ? 'is-invalid' : '' }}"
                                                    type="text" name="jam_buka" value="08:00">
                                            </div>
                                            @if ($errors->has('jam_buka'))
                                                <span class="text-danger">{{ $errors->first('jam_buka') }}</span>
                                            @endif
                                        </div>
                                        <div class="p-2">s/d</div>
                                        <div class="w-100">
                                            <div class="time-icon">
                                                <input
                                                    class="form-control timepicker {{ $errors->has('jam_tutup') ? 'is-invalid' : '' }}"
                                                    type="text" name="jam_tutup" value="16:00">
                                            </div>
                                            @if ($errors->has('jam_tutup'))
                                                <span class="text-danger">{{ $errors->first('jam_tutup') }}</span>
                                            @endif
                                        </div>
                                    </div>

                                </div>
                                <div class="text-center m-t-20">
                                    <button class="btn btn-primary submit-btn" type="submit">Add</button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            @endisset

            <div class="row">
                <div class="col-md-6">
                    <div class="card-box">
                        <div class="row">
                            <div class="col-sm-4 col-3">
                                <h4 class="card-title">Jadwal</h4>
                            </div>
                            <div class="col-sm-8 col-9 text-right m-b-20">
                                @if (empty(isset($_GET['add'])))
                                    <a href="{{ route('jadwal', ['id' => $dokter['id']]) }}?add"
                                        class="btn btn btn-primary btn-rounded float-right"><i class="fa fa-plus"></i> Add
                                        Jadwal</a>
                                @endif
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table table-striped custom-table">
                                        <thead>
                                            <tr>
                                                <th>Hari</th>
                                                <th>Jam</th>
                                                <th class="text-right">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($jadwals as $jadwal)
                                                <tr>
                                                    <td class="text-capitalize">{{ $jadwal['hari'] }}</td>
                                                    <td>
                                                        {{ date_format(date_create($jadwal['jam_buka']), 'H:i') }}
                                                        -
                                                        {{ date_format(date_create($jadwal['jam_tutup']), 'H:i') }}
                                                    </td>
                                                    <td class="text-right">
                                                        <div class="dropdown dropdown-action">
                                                            <a href="#" class="action-icon dropdown-toggle"
                                                                data-toggle="dropdown" aria-expanded="false"><i
                                                                    class="fa fa-ellipsis-v"></i></a>
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                <a class="dropdown-item"
                                                                    href="{{ route('jadwal.edit', ['id_dokter' => $dokter['id'], 'id_jadwal' => $jadwal['id']]) }}"><i
                                                                        class="fa fa-pencil m-r-5"></i> Edit</a>
                                                                <a class="dropdown-item" href="#" data-toggle="modal"
                                                                    data-target="#delete_jadwal{{ $jadwal['id'] }}"><i
                                                                        class="fa fa-trash-o m-r-5"></i>
                                                                    Delete</a>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
                                                <div id="delete_jadwal{{ $jadwal['id'] }}"
                                                    class="modal fade delete-modal" role="dialog">
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content">
                                                            <div class="modal-body text-center">
                                                                <img src="{{ asset('assets/img/sent.png') }}" alt=""
                                                                    width="50" height="46">
                                                                <h3>Anda yakin ingin menghapus Jadwal hari
                                                                    {{ $jadwal['hari'] }}?</h3>
                                                                <div class="m-t-20"> <a href="#" class="btn btn-white"
                                                                        data-dismiss="modal">Close</a>
                                                                    <form action="{{ route('jadwal.delete') }}"
                                                                        method="POST">
                                                                        @csrf
                                                                        <input type="hidden" name="id"
                                                                            value="{{ $jadwal['id'] }}">
                                                                        <button type="submit"
                                                                            class="btn btn-danger">Delete</button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    @endsection
